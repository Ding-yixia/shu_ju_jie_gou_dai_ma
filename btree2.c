...
typedef pBiTree ElemType; // 自定义栈的数据元素为二叉树。
...
// 求二叉树的高度。
int TreeDepth(pBiTree TT);

// 访问结点元素。
void visit(BiTNode *pp);

// 采用递归的方法对二叉树的先序遍历。
void PreOrder(pBiTree TT);
// 不采用递归的方法对二叉树的先序遍历。
void PreOrder1(pBiTree TT);

// 采用递归的方法对二叉树的中序遍历。
void InOrder(pBiTree TT);
// 不采用递归的方法对二叉树的中序遍历。
void InOrder1(pBiTree TT);

// 采用递归的方法对二叉树的后序遍历。
void PostOrder(pBiTree TT);
// 不采用递归的方法对二叉树的后序遍历。
void PostOrder1(pBiTree TT);
...
pBiTree CreateBTree()
{
  ...
  {
    TT = (pBiTree)malloc(sizeof(BiTNode));
    TT->data = ch;
    printf("请输入%c结点的左孩纸结点", TT->data);
    TT->lchild = CreateBTree();
    printf("请输入%c结点的右孩纸结点", TT->data);
    TT->rchild = CreateBTree();
  }
  return TT;
};
...
void PreOrder1(pBiTree TT)
{
  ...
  while ((ee != NULL) || (IsEmpty(&SS) != 1))//当第一个根节点不为空，栈空间未满时
  {
    if (ee != NULL)
    {
      visit(ee);       // 入栈前访问。
...
// 不采用递归的方法对二叉树的后序遍历。
void PostOrder1(pBiTree TT)//前序遍历
{
  SeqStack SS; // 创建顺序栈。
